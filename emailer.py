...
GRAPH_DRAFT_URL = "https://graph.microsoft.com/v1.0/users/{sender}/messages"
...
def _create_draft(payload: dict) -> str:
    token = _get_token()
    url = GRAPH_DRAFT_URL.format(sender=config.EMAIL_FROM)
    delays = [1, 4, 16]
    last_exc: Exception | None = None
    ...
    for attempt in range(1, 4):
        try:
            ...
            last_exc = exc
            logger.error(
                "Microsoft Graph draft error: %s %s. Attempt %d/3.",
                exc.response.status_code,
                exc.response.text[:200],
                attempt,
            )
            ...
            last_exc = exc
            logger.error(
                "Microsoft Graph draft unexpected error: %s. Attempt %d/3.", exc, attempt
            )
            if attempt < 3:
                time.sleep(delays[attempt - 1])

    raise RuntimeError("Microsoft Graph create draft failed after 3 attempts") from last_exc
...
def save_draft(
    ...
    logger.info(
        "[%s] Recap draft saved for: %s (draft_id=%s)",
        meeting_id,
        title,
        draft_id,
    )
